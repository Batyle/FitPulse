...
The same web app handles reminder scheduling and accounts. New account records are saved in the `Users` tab; returning users are matched by email. Passwords are salted and hashed before storage. The **Forgot password** option emails a six-digit code that expires after 15 minutes (the first use requires Apps Script permission to send email).
